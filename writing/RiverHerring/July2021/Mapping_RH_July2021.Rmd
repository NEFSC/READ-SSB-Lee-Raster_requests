...
<!-- 
This is the markdown document used to make the raster maps for the MAMFC River Herring Request.  It uses the offshoreWind package. I didn't get a shapefile from the MAFMC, so I am using the Herring FW7 as a placeholder.
...
Jason: 
My initial thought was looking at small mesh bottom trawl and mid-water trawl revenue maps, all species, (three time periods * 2 gears = 6 maps) that had the same 4 areas (green dashed) noted as my memo similarly outlined (I just drew them in over TMS lines).

    Small mesh OTF might be somewhat tricky. 



To fix this, you have to run the getCallArea on the input shapefile and changes some of these lines:
...
TO do: 
Figure out how to filter just the  small mesh BT.
...
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("OTM","PTM") &FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
...
Quitting from lines 1802-2143 (Mapping_RH_July2021.Rmd) 
Error in sort.int(x, na.last = na.last, decreasing = decreasing, ...) : 
  'x' must be atomic
Calls: <Anonymous> ... withVisible -> eval -> eval -> sort -> sort.default -> sort.int
...
logical_subset<-quote(which(FINALstack$GEARCODE %in% c("TRAWL-BOTTOM") &FINALstack$REGIME %in%c("1","2","3") & FINALstack[[FIELD]]>=1)) 
my_margin_name<-quote(paste(FINALstack$MONTH,readable_name, "REGIME",FINALstack$REGIME, sep="_"))
...
months<-c("9","10","11")
knitr::include_graphics(file.path(cleaned.image.path1,paste0("reclass_MWT.png")))
```
